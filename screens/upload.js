...
export default function Upload() {
  ...
  const fetchNurseData = async () => {
    try {
      const response = await fetch("http://192.168.204.148:2000/se/nurse");
  ...
  const submitForm = async () => {
    try {
      ...
      const scheduleResponse = await fetch("http://192.168.204.148:2000/se/schedule/store", {
        method: "POST",
        headers: {
      ...
      for (const nurse of selectedNurses) {
        ...
        await fetch("http://192.168.204.148:2000/se/assign/store", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
